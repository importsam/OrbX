...
if __name__ == '__main__':
    ...
    try:
        ...
        keplerian_df = keplerian_df[
            (keplerian_df["apogee"] >= 300) & (keplerian_df["apogee"] <= 700)
        ]
        ...
        if len(sat_nos) != len(labels):
            raise ValueError(
                f"label alignment mismatch: {len(sat_nos)} sat numbers vs {len(labels)} labels"
            )
        label_by_sat = {_norm_norad(s): int(lab) for s, lab in zip(sat_nos, labels)}
        results_df["label"] = results_df["NORAD_CAT_ID"].map(
