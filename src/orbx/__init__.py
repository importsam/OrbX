...
def __getattr__(name):
    # Lazy imports so `from orbx import cluster` works without Orekit.
    # Cache the result in globals() so we overwrite any submodule of the
    # same name (e.g. package ``orbx.density`` vs function ``density``).
    if name == "cluster":
    ...
    if name == "density":
        # Import the function via Density.py — ``from orbx.density import density``
        # would leave ``orbx.density`` bound to the package submodule.
