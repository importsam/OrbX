...
class ClusterWrapper:
    ...
    def run_all_optimizer(self, distance_matrix: np.ndarray, X: np.ndarray):
        ...
        optics_results = self.optics.run(
    ...
    def run_hdbscan(self, distance_matrix: np.ndarray, X: np.ndarray) -> np.ndarray:
        labels, best_score = self.hdbscan.run(distance_matrix.copy(), X.copy())
        print(f"HDBSCAN found {len(set(labels) - {-1})} clusters")
        print(f"HDBSCAN best score: {best_score:.4f}")
        self.quality_metrics.quality_metrics(X, distance_matrix, labels)
