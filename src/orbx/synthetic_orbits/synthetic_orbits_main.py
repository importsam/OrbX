...
from orbx.synthetic.orbit_finder.get_optimum_orbit import get_optimum_orbit, get_maximally_separated_orbit
from orbx.synthetic.data_handling.build_czml import build_czml
from orbx.synthetic.data_handling.ionop_czml import ionop_czml
...
from orbx.synthetic.orbit_finder.frechet_orbit_finder import get_optimum_orbit
from orbx.synthetic.orbit_finder.max_separation_orbit_finder import get_maximally_separated_orbit
...
class SyntheticOrbits:
    ...
    def preprocess_df(self) -> pd.DataFrame:
        ...
        # filter items was to pick out a single cluster for the paper. no need now.
        # df = self.filter_clusters(df.copy())
    ...
    # def filter_clusters(self, df: pd.DataFrame) -> pd.DataFrame:
    #     labels = df['label'].to_numpy()
    #     unique_labels, label_counts = np.unique(labels, return_counts=True)
    #     cluster_counts = dict(zip(unique_labels, label_counts))

    #     # choose the first non-noise cluster
    #     selected = [
    ...
    #     if not selected:
    #         raise ValueError(f"No clusters found in dataframe with labels: {unique_labels}")
    ...
    def run_orbit_generator(self, df, mode="max_separation_single"):
        
        """
        Here, there are two tools:
        1. Fréchet mean optimiser (get_optimum_orbit)
        This is the main tool for generating synthetic orbits that are "representative" of a cluster. 
        It finds the orbit that minimizes the average Fréchet distance to all orbits in the cluster. 
        This orbit is appended to the cluster with satNo=99999.
        
        2. Max-separation optimiser (get_maximally_separated_orbit)
        This is a secondary tool for generating synthetic orbits that are "maximally different"
        from a cluster. It finds the orbit that maximizes the minimum distance to any orbit in the cluster.
        
        Input:
        This takes in a df which at a minimum must contain a line1, line2, and a cluster label.
